...
from utils_rl import CustomMaskableActorCriticPolicy, parse_list_act_fn, parse_list_int_with_None, parse_list_bool, parse_list_float, parse_bool, parse_dic_args
...
class CustomMPPO(MaskablePPO):
    ...
    def _post_process_actions(self, env, actions, action_masks=None):
        ...
        if action_masks is not None:
            power_action_masks = np.ones((action_masks.shape[0], env.number_of_chargers, 2), dtype=int)
            zeros_indicies = list(np.where(power_masks == 0))
            zeros_indicies = tuple(zeros_indicies + [np.zeros(len(zeros_indicies[0]), dtype=int)])
            power_action_masks[zeros_indicies] = 0
            action_masks = np.concatenate((action_masks[:, :env.number_of_chargers], action_masks[:, env.number_of_chargers:] * power_action_masks), axis=1)
