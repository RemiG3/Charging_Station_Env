...
class CustomMPPO(MaskablePPO):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            actions, action_masks = self._post_process_actions(env, actions, action_masks)
    ...
    def predict(self, env, *args, **kwargs):
        ...
        if isinstance(env, VecEnv):
            processed_actions, _ = self._post_process_actions(env, actions)
        else:
            processed_actions, _ = self._post_process_actions(env, actions.reshape((1, -1)))
    ...
    def _post_process_actions(self, env, actions, action_masks=None):
        ...
        if action_masks is not None:
            power_action_masks = np.ones((action_masks.shape[0], number_of_chargers, 2), dtype=int)
            zeros_indices = list(np.where(power_masks == 0))
            if zeros_indices[0].size > 0:
                zeros_indices = tuple(zeros_indices + [np.zeros(len(zeros_indices[0]), dtype=int)])
                power_action_masks[zeros_indices] = 0
            action_masks = np.concatenate(
                (
                    action_masks[:, :number_of_chargers],
                    action_masks[:, number_of_chargers:] * power_action_masks,
                ),
                axis=1,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--folder_name", default="CustomMPPO", type=str)
