...
class CustomMPPO(MaskablePPO):
    ...
    def _post_process_actions(self, env, actions, action_masks=None):
        ...
        number_of_chargers = int(number_of_chargers_list[0])
        ...
        max_power_per_env = []
        for schema, energy, timestep in zip(schema_list, energy_list, timestep_list):
            step_time = schema.get('step_time', 1)
...
if __name__ == "__main__":
    ...
    if args.model_path_saved and os.path.exists(args.model_path_saved):
        model = CustomMPPO.load(
            args.model_path_saved,
            policy=CustomMaskableActorCriticPolicy,
            env=env,
            verbose=1,
        ...
        model = CustomMPPO(
            CustomMaskableActorCriticPolicy,
            env,
            verbose=1,
            tensorboard_log=logdir,
