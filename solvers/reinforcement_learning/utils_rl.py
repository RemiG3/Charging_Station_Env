...
def parse_list_act_fn(values):
    values = str(values).replace("'", '').replace('"', '').lower()
    list_val = values.split(',') if (',' in values) else [values]
    list_act_fn = []
    for val in list_val:
        if val == 'relu':
            list_act_fn.append( nn.ReLU )
        elif val == 'leakyrelu':
            list_act_fn.append( nn.LeakyReLU )
        elif val == 'tanh':
            list_act_fn.append( nn.Tanh )
        elif val == 'elu':
            list_act_fn.append( nn.ELU )
        elif val == 'gelu':
            list_act_fn.append( nn.GELU )
        elif val == 'sigmoid':
            list_act_fn.append( nn.Sigmoid )
        elif val == 'softplus':
            list_act_fn.append( nn.Softplus )
        elif val == 'none':
            list_act_fn.append( None )
        else:
            raise Exception(f'Activation function {val} not recognized')
    return list_act_fn[0] if (len(list_act_fn) == 1) else list_act_fn
...
def create_mlp(
    ...
    """
    Create a MLP, which is a collection of linear layers each followed by an activation function.
    ...
    if len(net_arch) > 0:
        ...
        if ((isinstance(nn_batchnorm, list) and nn_batchnorm[idx]) or ((not isinstance(nn_batchnorm, list)) and nn_batchnorm)):
            modules.append( nn.BatchNorm1d(num_features=net_arch[0]) )
        if ((isinstance(nn_dropout, list) and (nn_dropout[idx] > 0.)) or ((not isinstance(nn_dropout, list)) and (nn_dropout > 0.))):
            modules.append( nn.Dropout(p=nn_dropout[idx] if isinstance(nn_dropout, list) else nn_dropout) )
        if (not isinstance(activation_fn, list) and (activation_fn is not None)) or (isinstance(activation_fn, list) and (activation_fn[idx] is not None)):
            modules.append(activation_fn[idx]() if isinstance(activation_fn, list) else activation_fn())
    else:
        modules = []

    for idx in range(1, len(net_arch)):
        modules.append(nn.Linear(net_arch[idx - 1], net_arch[idx], bias=with_bias))
        if ((isinstance(nn_batchnorm, list) and nn_batchnorm[idx]) or ((not isinstance(nn_batchnorm, list)) and nn_batchnorm)):
    ...
    if output_dim > 0:
        ...
        if ((isinstance(nn_batchnorm, list) and (idx < len(nn_batchnorm)) and nn_batchnorm[idx]) or ((not isinstance(nn_batchnorm, list)) and nn_batchnorm)):
            modules.append( nn.BatchNorm1d(num_features=output_dim) )
        if ((isinstance(nn_dropout, list) and (idx < len(nn_dropout)) and (nn_dropout[idx] > 0.)) or ((not isinstance(nn_dropout, list)) and (nn_dropout > 0.))):
            modules.append( nn.Dropout(p=nn_dropout[idx] if isinstance(nn_dropout, list) else nn_dropout) )
        if ((not isinstance(activation_fn, list) and (activation_fn is not None)) or (isinstance(activation_fn, list) and (idx < len(activation_fn)) and (activation_fn[idx] is not None))):
            modules.append(activation_fn[idx]() if isinstance(activation_fn, list) else activation_fn())
    # if squash_output:
