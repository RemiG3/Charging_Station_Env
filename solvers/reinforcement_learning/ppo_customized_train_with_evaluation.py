...
class CustomPPO(PPO):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            clipped_actions = self._post_process_actions(env.envs[0], clipped_actions)
    ...
    def predict(self, env, *args, **kwargs):
        ...
        actions = self._post_process_actions(env, actions.reshape((1, -1)))
...
if __name__ == "__main__":
    ...
    if args.model_path_saved and os.path.exists(args.model_path_saved):
        model = CustomPPO.load(args.model_path_saved, policy=CustomActorCriticPolicy, env=env, verbose=1, tensorboard_log=logdir, learning_rate=args.learning_rate, n_steps=args.n_steps, batch_size=args.batch_size, n_epochs=args.n_epochs, gamma=args.gamma, gae_lambda=args.gae_lambda, clip_range=args.clip_range, clip_range_vf=None, normalize_advantage=args.normalize_advantage, ent_coef=args.ent_coef, vf_coef=args.vf_coef, max_grad_norm=args.max_grad_norm, target_kl=None, seed=args.seed, device='auto', _init_setup_model=True)
    else:
        model = CustomPPO(CustomActorCriticPolicy, env, verbose=1, tensorboard_log=logdir, policy_kwargs=policy_kwargs, learning_rate=args.learning_rate, n_steps=args.n_steps, batch_size=args.batch_size, n_epochs=args.n_epochs, gamma=args.gamma, gae_lambda=args.gae_lambda, clip_range=args.clip_range, clip_range_vf=None, normalize_advantage=args.normalize_advantage, ent_coef=args.ent_coef, vf_coef=args.vf_coef, max_grad_norm=args.max_grad_norm, target_kl=None, seed=args.seed, device='auto', _init_setup_model=True)
