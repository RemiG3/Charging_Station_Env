...
Result* solve(float alpha, float beta, float gamma, int current_ts, int T, int m, int nb_past, int nb_assigned, int nb_requests, float w, float eta, float w_G, float tau, float* b_ptr, int* d_j_ptr, int* r_j_ptr, float* soc_0_ptr, float* pv_ptr, float* pr_ptr,
    ...
    int n = nb_requests; //nb_past + nb_assigned + nb_requests;
    for(int j = 0; j < n; j++){
    ...
    for(int t = 0; t < T; t++) {
        name << "e_t_" << t;
        if(t < current_ts)
            e_t[t] = model.addVar(max(0.f, past_e_t[t]-epsilon), min(past_e_t[t]+epsilon, w_G*tau), obj_coef, GRB_CONTINUOUS, name.str());
        else
            e_t[t] = model.addVar(0., w_G*tau, obj_coef, GRB_CONTINUOUS, name.str());       // Constraint (13)
        name.str("");
    }
        ...
        for(int i = 0; i < m; i++){
            ...
            for(int j_ = 0; j_ < n; ++j_){
                if((j_ != j) && (((r_j[j_] < r_j[j]) && (d_j[j_] > d_j[j])) || ((r_j[j_] <= r_j[j]) && (d_j[j_] == d_j[j])))){
                    model.addConstr( y_ij[i][j] + y_ij[i][j_] <= 1 );
                }
            }
        }
...
//            model.addConstr( expr <= (((d_j[j] - r_j[j])*(d_j[j] - r_j[j])) / (((b[j] * (.8 - soc_0[j])) / (w * eta)) * tau + (d_j[j] - r_j[j]))) * y_ij[i][j]  +  T * (1 - y_ij[i][j]) );
        ...
        for(int i = 0; i < m; i++) {
            ...
            model.addConstr( expr1 <= T * y_ij[i][j] );                         // Constraint (8)
        }
        ...
        model.addConstr( soc_jf[j] == (soc_0[j] + (tau * w * eta * expr) / b[j]) ); // Constraint (9)
        model.addConstr( soc_jf[j] <= 1. - .20000001 * (1 - u_j[j]) );              // Constraint (10)
        model.addConstr( soc_jf[j] >= .8 * u_j[j] );                                // Constraint (11)
    }
    ...
    for(int t = 0; t < T; t++){
        ...
        model.addConstr( e_t[t] >= tau * (expr1 * w - pv_t[t]) );                   // Constraint (12)
        model.addConstr( expr <= 2 * m );                                           // Constraint (14)
    }
            ...
            if (res->x_ijt[i] != nullptr) {
                for (int j = 0; j < n; j++) {
                    if (res->x_ijt[i][j] != nullptr) {
                        free(res->x_ijt[i][j]); // Free the array of T booleans
                        res->x_ijt[i][j] = nullptr; // Set to nullptr after freeing
                    }
                }
                free(res->x_ijt[i]); // Free the array of n pointers to arrays of T booleans
                res->x_ijt[i] = nullptr; // Set to nullptr after freeing
            }
        }
        free(res->x_ijt); // Free the array of m pointers to arrays of n pointers
        res->x_ijt = nullptr; // Set to nullptr after freeing
    }
    if (res->y_ij != nullptr) {
        for (int i = 0; i < m; i++) {
            if (res->y_ij[i] != nullptr) {
                free(res->y_ij[i]); // Free the array of n booleans
                res->y_ij[i] = nullptr; // Set to nullptr after freeing
            }
        }
        free(res->y_ij); // Free the array of m pointers to arrays of n booleans
        res->y_ij = nullptr; // Set to nullptr after freeing
    }
    if (res->soc_jf != nullptr) {
        free(res->soc_jf);
        res->soc_jf = nullptr; // Set to nullptr after freeing
    }
    if (res->e_t != nullptr) {
        free(res->e_t);
        res->e_t = nullptr; // Set to nullptr after freeing
    }
    if (res->u_j != nullptr) {
        free(res->u_j);
        res->u_j = nullptr; // Set to nullptr after freeing
    }
    free(res);
    res = nullptr; // Set to nullptr to avoid dangling pointer
}
