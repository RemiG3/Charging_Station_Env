...
class Rolling:
    ...
    def predict(self, state):
        ...
        horizon = abs(nb_predict) + 1

        assert len(state) >= 2 * horizon, f"Observation too short for configured horizon (len={len(state)}, horizon={horizon})"
        price_segment = state[:horizon]
        pv_segment = state[horizon:2 * horizon]
        price_norm = self.env.energy["normalizers"]["price"]
        pv_norm = self.env.energy["normalizers"]["renewable"]

        if nb_predict >= 0:
            for offset in range(horizon):
                target_ts = self.ts + offset
                if target_ts >= self.T:
                    break
                price_val = price_segment[offset]
                pv_val = pv_segment[offset]
                if price_val >= 0:
                    self.pr_t[target_ts] = price_val * price_norm
                    if self.pr_t[target_ts] > self.max_pr_known:
                        self.max_pr_known = self.pr_t[target_ts]
                if pv_val >= 0:
                    self.pv_t[target_ts] = pv_val * pv_norm
        else:
            current_price = price_segment[-1]
            current_pv = pv_segment[-1]
            if current_price >= 0:
                self.pr_t[self.ts] = current_price * price_norm
                if self.pr_t[self.ts] > self.max_pr_known:
                    self.max_pr_known = self.pr_t[self.ts]
            if current_pv >= 0:
                self.pv_t[self.ts] = current_pv * pv_norm
...
if __name__ == "__main__":
    ...
    parser.add_argument("--overwrite_results", default=0, type=int) # 0: skip if result exists, 1: overwrite existing results
    ...
    for ep in range(episodes):
        result_path = os.path.join(results_folder, f"{algo_name}-Results-{ep+1}.pickle")
        if (args.overwrite_results == 0) and os.path.exists(result_path):
            print(f'Skip episode {ep+1}: result exists -> {os.path.basename(result_path)}', end='\r', flush=True)
            continue
        done = False
