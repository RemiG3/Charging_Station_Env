...
IloModel create_model_p1(IloEnv const &env, int T, int m , int n, double w, double eta, double w_G, double tau, vector<int> const&r_j, vector<int> const&d_j, vector<float> const&soc_0, vector<float> const&b, vector<float> const&pv_t, vector<float> const&pr_t,
                         IloArray<IloBoolVarArray> &x_jt, IloArray<IloBoolVarArray> &z_jk, IloBoolVarArray &y_j, IloNumVarArray &q_j, IloNumVarArray &floor_qj){
    try {
        IloModel mod ( env );
        stringstream name;
...
IloModel create_model_p2(IloEnv const &env, int T, int m , int n, double w, double eta, double w_G, double tau, vector<float> yj_assigned, vector<vector<bool>> zjk_assigned, vector<float> soc_jf_assigned, vector<int> const&r_j, vector<int> const&d_j, vector<float>  const&soc_0, vector<float> const&b, vector<float> const&pv_t, vector<float> const&pr_t,
...
void write_log_file(IloEnv env, IloCplex cplex, string output_filename, int n, int m, int T, double w, double eta, double w_G, float tau, vector<vector<float>> yij_assigned, vector<int> const&r_j, vector<int> const&d_j, vector<float>  const&soc_0, vector<float> const&b, vector<int> const&charger, vector<float> const&pv, vector<float> const&pr,
                    ...
                    IloNum total_time, bool solved, double obj_val, double gap_p1, double gap_p2){
    string dir_path = get_current_working_dir();
...
void run_model(int nb_ev, int ev_scenario, int pr_scenario, int pv_scenario, int station_scenario, bool use_charger_assignement, string output_filename){
    ...
    int T = int(round(24*1/tau));
    vector<float> soc_f;
    vector<vector<float>> yij_assigned; // For post-precossing
    vector<float> yj_assigned;
    vector<vector<bool>> zjk_assigned;
    vector<float> soc_jf_assigned;
    ...
    IloNum elapsed_time_p1 = 0., elapsed_time_p2 = 0.;
    ...
    mod = create_model_p1(env, T, m, n, w, eta, w_G, tau, r_j, d_j, soc_0, b, pv, pr, x_jt, z_jk, y_j, q_j, floor_qj);
    IloCplex cplex1(mod);
    ...
    try {
        ...
        if (solved){
            ...
            for(int j = 0; j < n; j++) {
                zjk_assigned.push_back(* new vector<bool>());
                if(j >= 1) {
                    cplex1.getValues(vals, z_jk[j]);
                    for(int k = 0; k < j; ++k) {
                        if(int(round(vals[k])) == 1) {
                            zjk_assigned[j].push_back( true );
                        } else {
                            zjk_assigned[j].push_back( false );
                        }
                    }
                }
            }

            mod = create_model_p2(env, T, m, n, w, eta, w_G, tau, yj_assigned, zjk_assigned, soc_jf_assigned, r_j, d_j, soc_0, b, pv, pr, x_jt, soc_jf, e_t, u_j, v_j, q_j, floor_qj, z_jk);
            IloCplex cplex(mod);

            cplex.setParam(IloCplex::Param::TimeLimit, timeout2);
            IloNum start_time = cplex.getCplexTime();
            solved = cplex.solve();
            elapsed_time_p2 = (cplex.getCplexTime()-start_time);

            solved_log = solved_log && solved && (elapsed_time_p2 < timeout2);
            ...
            if(solved) {
                ...
                write_log_file(env, cplex, output_filename, n, m, T, w, eta, w_G, tau, yij_assigned, r_j, d_j, soc_0, b, charger, pv, pr,
                               x_jt, soc_jf, e_t, y_j,
                               elapsed_time_p1+elapsed_time_p2, solved_log, (double)cplex.getBestObjValue(), (double)cplex1.getMIPRelativeGap(), (double)cplex.getMIPRelativeGap());
