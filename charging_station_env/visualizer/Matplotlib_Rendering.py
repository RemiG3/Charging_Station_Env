...
class Matplotlib_Rendering(Rendering_Base):
    ...
    def __call__(self, env, obs):
        ...
        # Use grid limit from schema if present and positive
        grid_limit_val = env.schema.get('grid_limit', None)
        if isinstance(grid_limit_val, (int, float)) and (grid_limit_val > 0):
            grid_limit = np.repeat([grid_limit_val * (env.step_time / 60)], len(grid_history))
        else:
            grid_limit = None
