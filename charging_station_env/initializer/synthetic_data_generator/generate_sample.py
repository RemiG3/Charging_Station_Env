...
import pandas as pd
import numpy as np
from charging_station_env.initializer.synthetic_data_generator.handles.data_hand import create_factor_arr
import random


def generate_sample(AM, MMc, MMe, horizon_start, horizon_end, n=50):
    ...
    for i in range(0, number_of_wd):
        ...
        arrivals, t_next,_ = AM.predict_day(X_test=X_test, Start_time=np.array(start), slot=int(AM.slotmin)*60,
                                            variablity_lambda=True, verbose=0)
        arrivals = arrivals.reshape(-1, 1)
        if(len(arrivals)+current_n > n):
            idx = [random.randint(0, len(arrivals)-1) for _ in range(n-current_n)]
            arrivals = arrivals[idx].reshape(-1, 1)
            current_n = n
        else:
            current_n += len(arrivals)
        
        deps = MMc.predict_day(X_test=X_test, arrivals=np.array(arrivals), slot=int(AM.slotmin)*60, verbose=0)#config['verbose'])
        deps = deps.reshape(-1, 1)
        energy_req = MMe.predict_day(X_test=X_test, arrivals=np.array(arrivals), slot=int(AM.slotmin)*60, verbose=0)#config['verbose'])
