...
"""
class Initializer(Initializer_Base):
    ...
    def _scenario_init(self, env: Type[gym.Env]) -> Dict:
        #assert(env.chargers_config['charging_mode'] == 'discrete'), 'The charging station should have discrete chargers'
        ...
        for charger_type in env.schema['Charger_types']:
            chargers_ev_compatibilities[charger_type] = []
            for ev_num, ev_type in enumerate(env.schema['EV_config']['considered_ev']):
                if charger_type in env.schema['EV_types'][ev_type]['chargers_type_compatibilities']:
        ...
        restart = True
        df = None
        horizon_start = datetime.datetime.strptime('01/01/2015', '%d/%m/%Y') + ( datetime.timedelta(days=random.randint(1, 364)) if self.select_day_randomly else datetime.timedelta(days=self.day) )
        ...
        charger_type = env.schema['Charger_types'][env.schema['Chargers_config']['list_chargers'][0]]
        charging_rate = max(charger_type['charging_rate']) if isinstance(charger_type['charging_rate'], list) else charger_type['charging_rate']
        charging_efficiency = max(charger_type['charging_efficiency']) if isinstance(charger_type['charging_efficiency'], list) else charger_type['charging_efficiency']
        p = charging_rate * charging_efficiency
        min_dur = (self.b_max * (soc_jd - initial_socs)) / p * (60 // step_time)
        ...
        for soc, arr, dep, typ in zip(initial_socs, arrivals_requests, departures_requests, ev_types_requests):
            charging_requests[arr].append({'soc': soc, 'soc_d': soc_jd, 'arr': arr, 'dep': dep, 'type': typ})
