...
class Simulate_Actions_FIFO(Simulate_Actions_Base):
    ...
    def _process_charging_actions(self, env, actions, ts):
        ...
        grid_penalty = 0
        if env.schema['grid_limit'] > 0.:
            grid_cap_kwh = env.schema['grid_limit'] * (env.step_time / 60)
            tolerance = max(1e-5, 1e-6 * grid_cap_kwh)
            # if grid_final - grid_cap_kwh > tolerance:
            #     assert False, f'Grid limit reached: {grid_final} > {grid_cap_kwh}'
            if grid_final > grid_cap_kwh:
                grid_penalty = self.alpha_grid_limit
                grid_final = grid_cap_kwh
                # Respect efficiency when enforcing grid limit: scale by consumed energy
                sum_consumed = float(np.sum(consumed_energies))
                target_total_consumed = grid_cap_kwh + renewable_energy
                if sum_consumed <= 1e-12:
                    p_charging[:] = 0.0
                    consumed_energies[:] = 0.0
                    total_consumed_energy = renewable_energy
                    grid_final = 0.0
                else:
                    scale = target_total_consumed / sum_consumed
                    p_charging *= scale
                    consumed_energies *= scale
                    total_consumed_energy = float(np.sum(consumed_energies))
                    grid_final = max(total_consumed_energy - renewable_energy, 0.0)
                price = self.alpha_price * grid_final * env.energy["price"][ts]
                energy_wasted = max(renewable_energy - total_consumed_energy, 0.0)
